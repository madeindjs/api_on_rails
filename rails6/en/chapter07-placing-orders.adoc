...
Why did you put `inverse_of` option in `Placement#product` relation?::
  . this help Rails to determine `Product#placements` relation
  . we need to
...
How can we add a `product` to an `order`::
...
How can we ensure than an Active Record field is greater than 0:: `validates :total, numericality: { greater_than_or_equal_to: 0 }`


Why did you put `inverse_of` option in `Placement#product` relation?:: this help Rails to determine `Product#placements` relation. This can be omited and our application would continue to works. This option could help you to defines relations when relation name is different than class name.
...
What is the best Active Record callback to compute order's total?:: `before_validation`. We need to do so because we set a validation on total so `before_save` would not works. Also you can take a look at https://guides.rubyonrails.org/active_record_callbacks.html#available-callbacks[available Active Record callback].
