...
What is the purpose of `Hash#dish` method?::
  . It allow
  . It is a Ruby method allowing you to retrieve elements in a nested _Hash_ by avoiding errors if an element is not present.
...
Which statement is not correct about scope with Active Record?::
  . this is a shorter way to write a query
  . this may improve performances
  . this allow us to chain conditions

Instead of returning an array of all the movies that match this criteria, using a scope actually returns an ActiveRecord Relation.
