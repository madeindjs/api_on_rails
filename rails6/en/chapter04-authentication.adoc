...
What may happen if we try to use `JWT.decode` with an invalid signature? Exemple, we define a token `token = JWT.encode({}, 'bar')` and then decoded like this `JWT.decode(token, 'wrong')`.::
