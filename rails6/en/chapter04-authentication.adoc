...
Which of these snippets can be a valid JWT header::
+
[source,json]
----
{ "alg": "HS256", "typ": "JWT" }
...
Which of these snippet send a correct HTTP query in a functional test::
...
Which of theses snippet can be a valid JWT header:: `{ "alg": "HS256", "typ": "JWT" }`. JWT Header describes the cryptographic operations applied to the JWT and optionally, additional properties of the JWT. https://tools.ietf.org/html/rfc7519#section-5[see RFC 7519].

How do you build a valid JWT token with the given payload `payload = {email: 'john@doe.org'}`?:: `JWT.encode(payload, 'my_secret_key')`. `encode` need at least a secret key to build a JWT token. Also please note that the secret key can be an empty string `""`.

What may happen if we try to use `JWT.decode` with an invalid signature?:: an error `JWT::VerificationError` will be raised. You may `rescue` it in your controller to handle it and send a `403 - Forbidden` to the user. This may happen if a malicious user tries to build a custom JWT token.
...
If you read carefully quiz's answer, you may notice that if we try to decode an invalid token JWT library raises an exception. At this point, we do not handle this exception in the application.

If you want to go further and improve your application at this point, you may want to handle properly this error and return a `403 - Forbidden` HTTP status and a body containing a message like "Given token is invalid because it has perished / malformed.". You can easily implement a functional test like the following:
