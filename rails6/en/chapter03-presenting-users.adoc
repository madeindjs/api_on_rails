...
Which code allow you to set an unique constraint on `User#email` in a migration file?::  `t.index :email, unique: true`

How do we create an user in a unit test?:: Add a record in `test/fixtures/users.yml`
  . use `Use.create email: 'toto@toto.fr'` inside a test
