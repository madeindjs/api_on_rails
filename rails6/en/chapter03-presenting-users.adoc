...
Which code allow you to set an unique constraint on `User#email` in a migration file?::  `t.index :email, unique: true`. This will create a brand new index with a constraint on `email` property.

How do we create an user in a unit test?:: Add a record in `test/fixtures/users.yml`. You could also use `Use.create email: 'toto@toto.fr'` inside a test but with fixture you ensure that this user exists for all you tests suites.

Which statement is not valid?:: Hash allow you to find the original character string. This is the real difference between hashing and encrypt a password.

Which feature `has_secure_password` no implement?:: The password must contains at least on letter and one digit. But you can easily add https://guides.rubyonrails.org/active_record_validations.html[an Active Record Validation] to do this.
