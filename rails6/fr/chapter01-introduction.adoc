...
Il faut néanmoins configurer les informations de l'auteur des _commits_. Si ce n'est pas déjà fait, placez vous dans le répertoire et lancez commandes suivantes:
...
L’étape suivante est d’ignorer certains fichiers que nous ne voulons pas suivre. Rails l'a fait pour nous en générant un fichier `.gitignore` qui devrait ressembler à celui montré ci-dessous:
