...
FastJSON API utilise des *sérialiseurs*. Les sérialiseurs représentes des classes Ruby qui seront responsables de convertir un modèle en un `Hash` ou un JSON.
...
C’est une bonne idée d’intégrer les utilisateurs dans les sortie JSON des produits. Cela rendra la sortie plus lourde mais ça évitera au client de l'API d'éxecuter d'autres requêtes pour récupérer les informations des utilisateurs liés aux produits. Cette méthode peut vraiment vous éviter un énorme goulet d'étranglement.
...
Les tests suivants s'assurent que la méthode `Product.filter_by_title` va rechercher correctement les produits en fonction de leurs titre. Nous utilisons le terme `tv` en minuscule afin de s'assurer que notre recherche ne sera pas sensible à la casse.
