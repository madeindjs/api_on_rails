...
Créer des produits est un peu plus délicat parce que nous aurons besoin d’une configuration supplémentaire. La stratégie que nous suivrons est d’attribuer le produit créé à l'utilisateur propriétatire du jeton JWT fourni d'en l'en-tête HTTP `Authorization`.
...
Et maintenant vous pouvez créer l'utilsateur en éxecutant simplement la commande suivante:
