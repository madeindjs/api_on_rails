...
Comme vous pouvez déjà l’imaginer, il existe de nombreuses solutions d’authentification pour Rails comme https://github.com/binarylogic/authlogic[AuthLogic], https://github.com/thoughtbot/clearance[Clearance] et https://github.com/plataformatec/devise[Devise]. Nous ne les utiliserons pas afin de mieux apréhender le mécanisme d'authentification. Nous utiliserons néamoins la gemme https://github.com/codahale/bcrypt-ruby[bcrypt] qui nous permettra de hasher les mots de passe de l'utilisateur.
...
Nous devons d’abord ajouter la gemme Bcrypt au _Gemfile_. Nous pouvons utiliser la commande `bundle add` 
