...
- nous vérifion qu'un utilisateur avec un courriel invalide n'est pas valide:
...
- nous vérifion qu'un utilisateur avec un courriel déjà utilisé n'est pas valide. Nous utilisons donc le même courriel que la _fixture_ que nous venons de créer.
...
// NOTE: La liste complète des _assertions_ disponibles sont listées dans la https://guides.rubyonrails.org/testing.html#available-assertions[documentation officielle]
