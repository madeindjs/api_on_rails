...
Cette première version était initialement prévue pour Ruby on Rails 4. Cette version ne https://guides.rubyonrails.org/maintenance_policy.html#security-issues[recevant plus les mise à jour de sécurité], j’ai voulu mettre à jour cet excellent ouvrage et contribuer à la communauté francophone en le traduisant moi-même.

NOTE: Cette mise à jour est aussi disponible dans la langue de Shakespeare.

Cette version est donc une amélioration de la version que j'ai écrite pour Rails 5 et elle se concentre sur Rails 6. Dans cette version je n'ai pas seulement cherché à rendre les exemples compatible mais j'ai aussi voulu simplifier la mise en place d'une API avec Rails.
