...
\chapter{Modélisation du JSON avec Active Model Serializers}\label{chapter:7}
  ...
  \begin{scriptsize}
    \begin{lstlisting}[language=json, caption={Exemple de la sortie JSON actuelle pour la route products}, label={lst:products_json}]
    {
      "products": [
          {
              "id": 1,
  ...
  \begin{scriptsize}
    \begin{lstlisting}[language=json, caption={Exemple d'une belle sortie JSON pour la route products}, label={lst:wanted_products_json}]
    {
      "products": [
          {
              "id": 1,
  ...
  \section{Serialiser l'utilisateur}
    ...
    Ceci a créé un fichier appelé \verb|user_serializer.rb| sous le répertoire \verb|app/serializers|, qui devrait ressembler à celui de Listing 7.2:
